...
const toggleLayer = (layer: keyof typeof selectedLayers.value) => {
  ...
  // In futuro, qui manderemo un segnale alla mappa per farle disegnare davvero il meteo o il traffico!
}
</script>
  ...
  <div class="absolute top-0 left-0 right-0 p-4 md:p-6 pointer-events-auto z-10">
    <div class="max-w-3xl mx-auto flex flex-col sm:flex-row gap-3">
      ...
      <div class="flex-1 relative">
        ...
        <input type="text" placeholder="Search locations in Trento..." class="w-full pl-10 pr-4 py-2.5 text-sm bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border border-cyan-500/30 dark:border-cyan-400/30 rounded-xl shadow-lg focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all text-slate-900 dark:text-slate-100" />
      </div>

      <div class="relative">
        <button @click="showLayersMenu = !showLayersMenu" class="w-full flex items-center gap-2 px-5 py-2.5 text-sm bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border border-cyan-500/30 dark:border-cyan-400/30 rounded-xl shadow-lg transition-all text-slate-900 dark:text-slate-100">
