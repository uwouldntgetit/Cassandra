...
// 1. Nuova variabile per tracciare se non ci sono risultati
...
// 2. Quando l'utente digita qualcosa, resettiamo lo stato di "nessun risultato"
const handleInput = () => {
...
const handleSearch = async () => {
  ...
  noResults.value = false // Resettiamo ad ogni nuova ricerca
  searchResults.value = [] 
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      // 3. Se la lista è vuota, attiviamo l'avviso
...
// Funzione per chiudere tutto cliccando fuori
  ...
  <div class="absolute top-0 left-0 right-0 p-4 md:p-6 pointer-events-auto z-10">
    <div class="max-w-3xl mx-auto flex flex-col sm:flex-row gap-3">
      ...
      <div class="flex-1 relative group">
        ...
        <!-- TENDINA: Si apre sia se ci sono risultati, sia se c'è l'errore "Nessun risultato" -->
