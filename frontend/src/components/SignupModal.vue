...
/**
 * Registration modal for new users: validates input and creates a new
 * session via the auth store.
 */
...
const handleSignup = async () => {
  ...
  try {
    ...
    errorMessage.value = e.message || 'Registrazione non riuscita. Riprova.'
  } finally {
        ...
        <div class="p-8 space-y-4">
          ...
          <div class="space-y-1.5">
            ...
            <div class="relative">
              ...
              <input v-model="password" type="password" placeholder="Min. 6 caratteri" class="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all text-sm text-slate-900 dark:text-slate-100" />
            </div>
          </div>

          <!-- Confirm Password Field -->
          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400 ml-1">Conferma password</label>
            <div class="relative">
              <Lock class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-cyan-500" />
              <input v-model="confirmPassword" type="password" placeholder="Ripeti la password" class="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all text-sm text-slate-900 dark:text-slate-100" />
            </div>
          </div>
          ...
          <p class="text-center text-xs text-slate-500 dark:text-slate-400 mt-4">
            Hai già un account? <button @click.prevent="emit('switch-to-login')" class="text-cyan-600 dark:text-cyan-400 font-bold hover:underline">Accedi</button>
          </p>
        </div>
      </div>
    </div>
